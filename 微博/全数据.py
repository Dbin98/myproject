...
base_url='''
https://m.weibo.cn/api/container/getIndex?containerid=100808abf16b190b1e62990c5d0a185cab7b57_-_sort_time&luicode=10000011&lfid=100103type=1&q=中南民族大学&since_id=4612045275926832
'''
        ...
        try:
            ...
            for item in data:
                ...
                if comments_count > 0:
                    ...
                    commend = []
            inf.append([userid, username, send_time, content, reposts_count, comments_count, attitudes_count, commend])
            base_url = re.sub(pat, 'since_id=' + str(since_id), base_url)
            print('第{}页写入完毕'.format(page))
            time.sleep(random.randint(3, 5))
        except:
            print('未爬到数据')
    inf1 = pd.DataFrame(inf, columns=['用户id', '昵称', '发布时间', '内容', '转发数', '评论数', '点赞数', '评论'])
    inf1.to_csv('all_daily_comment.csv', index=False, encoding='gb18030')
    print('爬虫结束')
